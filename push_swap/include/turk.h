...
/*   Updated: 2025/05/01 11:25:56 by saalarco         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 * to bring it to its target position in stack_b. 
 * (here we play with index + is_upper_half)
 * Case when number is right upper half in a and right 
 * down half in b that would benefit
 * from rrr or rr are not contemplated, we only measure 
 * up or down half.
 ...
 */
void	push_from_b_to_a(t_stacks *stacks);
...
// From info index + is_upper_half + is_cheapest
void	do_cheapest_move_b_to_a(t_stacks *stacks);

// finally
void	rotate_stack_a_min_to_top(t_stacks *stack_a);
